            ...
            <div className="grid grid-cols-1 gap-6">
                {filteredCode.length > 0 ? (
                    filteredCode.map((code, index) => (
                        <StatusCode key={code.code} code={code} index={index} />
                    ))
                ) : (
...
interface StatusCodeProps {
    code: StatusCodeDetails;
    index: number;
}

function StatusCode({ code, index }: StatusCodeProps) {
    return (
        <details
            tabIndex={index + 3}
            className="code-container group bg-white transition-all overflow-hidden"
        >
            <summary className="-z-50 cursor-pointer flex items-center justify-between p-4 bg-gray-100 hover:bg-gray-200 transition-colors">
            ...
            <div className="p-4 border toggle">
                ...
                <p className="text-gray-500 mt-2">
                    <span className="font-semibold">Example:</span> {code.example}
                </p>
            </div>
        </details>
    );
